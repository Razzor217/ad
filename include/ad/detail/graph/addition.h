...
template <node L, node R> class addition_impl {
public:
    using lhs_type = L;
    using rhs_type = R;
    ...
    explicit addition_impl(lhs_pointer const& lhs, rhs_pointer const& rhs)
        : m_lhs { std::move(lhs) }
        , m_rhs { std::move(rhs) }
...
template <node L, node R> using addition_ptr = std::shared_ptr<addition_impl<L, R>>;

template <node L, node R>
auto operator+(std::shared_ptr<L> const& lhs, std::shared_ptr<R> const& rhs) -> addition_ptr<L, R>
{
    return std::make_shared<addition_impl<L, R>>(lhs, rhs);
}
template <node L>
    requires arithmetic<typename L::value_type>
auto operator+(std::shared_ptr<L> const& lhs, typename L::value_type const rhs)
    -> addition_ptr<L, constant_impl<typename L::value_type>>
{
    return std::make_shared<addition_impl<L, constant_impl<typename L::value_type>>>(
...
auto operator+(typename R::value_type const lhs, std::shared_ptr<R> const& rhs)
    -> addition_ptr<constant_impl<typename R::value_type>, R>
{
    return std::make_shared<addition_impl<constant_impl<typename R::value_type>, R>>(
